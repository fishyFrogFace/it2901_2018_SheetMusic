...
class Setlist extends Component {
    ...
    _onDragEnd = async result => {
        ...
        await firebase
            .firestore()
            .doc(`bands/${band.id}/setlists/${setlist.id}`)
            .update({
                items: newItems,
            });
    };
            ...
            switch (type) {
                ...
                case 'addEvent':
                    const {
                        ...
                        description,
                        time,
                    } = await this.addEventDialog.open();
                    await setlistRef.update({
                        items: [
                            ...(setlist.items || []),
                            {
                ...
                case 'editSetlist':
                    const {
                        ...
                        date,
                        setlistTime,
                    } = await this.editSetlistDialog.open(setlist);
                    await setlistRef.update({
                        title: title,
                        date: date,
                        time: setlistTime,
                ...
                case 'download':
                    ...
                    Promise.all(array).then(function(values) {
                        Promise.all(instrumentPromiseArray).then(function(
                            instrumentArray
                        ) {
                            let instIndex = 0;
                            overview.forEach(item => {
                                //Add instrument to overview
                                if (item.type === 'score') {
                                    item.title +=
                                        ' (' +
                                        instrumentArray[instIndex++] +
                                        ')';
                                }
                            });
                                ...
                                array.forEach(items => {
                                        ...
                                        if (
                                            ...
                                            items.forEach(item => {
                                                ...
                                                doc.addImage(
                                                    ...
                                                    0,
                                                    0,
                                                    a4_size[0] / size_increase,
                                                    a4_size[1] / size_increase
                                                );
                                                doc.text(
                                                    `${dateString}     ${
                                                        setlist.title
                                    ...
                                    //Item is event
                                        ...
                                        while (desc.length > 0) {
                                            ...
                                            let lineToBeAdded = desc.substring(
                                                0,
                                                linelen
                                            );
                                            desc = desc.substring(
                            ...
                            overview.forEach(item => {
                                switch (item.type) {
                                    ...
                                    case 'score':
                                        let currentInstrument = item.title
                                            .split('(')[1]
                                            .split(')')[0];
                                        if (
                                            selectedInstrument ===
        ...
        if (page !== prevProps.page) {
            ...
            this.unsubs.push(
                bandRef.collection('scores').onSnapshot(async snapshot => {
                    ...
                    this.setState({
                        band: { ...this.state.band, scores: scores },
                    });
                })
            );
    ...
    _onEventDeleteClick = async (eventId, eventTitle) => {
        ...
        const setlistRef = firebase
            .firestore()
            .doc(`bands/${bandId}/setlists/${setlistId}`);
    ...
    _onEventEditClick = async (eventId, index) => {
        ...
        const {
            ...
            description,
            time,
        } = await this.editSetlistEventDialog.open(event);
    ...
    render() {
        ...
        return (
            <div className={classes.root}>
                        ...
                        <Toolbar>
                            ...
                            <div style={{ marginLeft: 10 }}>
                                ...
                                >
                                    ...
                                    {setlist.date &&
                                        this._formatedDate(setlist.date)}
                                    {setlist.time &&
                                        this._formatedTime(setlist.time)}
                                </Typography>
                            </div>
                            ...
                            {hasRights && (
                                <IconButton
                                    color="inherit"
                                    aria-label="Menu"
                                    onClick={e => this._onAddButtonClick(e)}
                                >
                            ...
                            >
                                <MenuItem
                                    id="add-score-menu-button"
                                    onClick={() =>
                                        this._onMenuClick('addScore')
                                    }
                                >
                                    Add Score
                                </MenuItem>
                                <MenuItem
                                    id="add-event-menu-button"
                                    onClick={() =>
                                        this._onMenuClick('addEvent')
                                    }
                                >
                                    Add Event
                                </MenuItem>
                                                                    ...
                                                                    >
                                                                        ...
                                                                        >
                                                                            ...
                                                                            {hasRights && (
                                                                                ...
                                                                                >
                                                                                    <DeleteIcon
                                                                                        id="score-delete-button"
                                                                                        onClick={() =>
                                                                                            this._onScoreDeleteClick(
                                                                                                item.id,
                                                                                                item
                                                                                                    .score
                                                                                                    .title
                ...
                />
                <AddSetlistScoresDialog
                    band={band}
                    onRef={ref => (this.addScoreDialog = ref)}
                />
                <AddSetlistEventDialog
                    onRef={ref => (this.addEventDialog = ref)}
                />
                <EditSetlistDialog
                    onRef={ref => (this.editSetlistDialog = ref)}
                />
                <EditSetlistEventDialog
                    onRef={ref => (this.editSetlistEventDialog = ref)}
                />
