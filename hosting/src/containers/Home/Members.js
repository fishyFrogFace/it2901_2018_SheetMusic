...
class Members extends React.Component {
    ...
    _onAccept = async member => {
        ...
        const pendingRef = firebase
            .firestore()
            .doc(`bands/${this.props.band.id}/pending/${member.id}`);
    ...
    _onDeleteBand = async () => {
        ...
        if (this.state.isLeader && band.creatorRef.id === this.state.user) {
            ...
            let userBandRefs = (await userRef.get()).data().bandRefs || [];
            let filteredRefs = await userBandRefs.filter(
                ref => ref.id !== bandRef.id
            );
            ...
            await band.members.map(item => {
                ...
                memberRef.get().then(doc => {
                    ...
                    let filteredRefs = memberBandRefs.filter(
                        ref => ref.id !== bandRef.id
                    );
            ...
            await bandRef
                ...
                .then(scores => {
                    scores.forEach(score => {
                        this._onDeleteCollection(
                            bandRef.collection(`scores/${score.id}/parts`),
                            50
                        );
                    });
                });
    ...
    _onLeave = async member => {
        ...
        const memberRef = firebase
            .firestore()
            .doc(`bands/${this.props.band.id}/members/${member.id}`);
        const userRef = firebase.firestore().doc(`users/${member.uid}`);
    ...
    _onLeaveLeader = async person => {
        const bandRef = firebase.firestore().doc(`bands/${this.props.band.id}`);
        const leaderRef = firebase
            .firestore()
            .doc(`bands/${this.props.band.id}/leader/${person.id}`);
    ...
    componentDidMount() {
        ...
        firebase
            ...
            .then(snapshot => {
                const admins =
                    snapshot.data() === undefined ? {} : snapshot.data().admins;
                const leader =
                    snapshot.data() === undefined
                        ? null
                        : snapshot.data().creatorRef.id;
        ...
        if (this.state.isLeader) {
            return (
                <div>
                        ...
                        {band.leader && band.leader.length > 0 && (
                            <Paper style={{ width: 500 }}>
                                ...
                                >
                                    ...
                                    >
                                        <MenuItem
                                            id="change-bandName-button"
                                            onClick={this._onChangeBandName}
                                        >
                                            Change bandname
                                        </MenuItem>
                                        <MenuItem
                                            id="change-bandType-button"
                                            onClick={this._onChooseBandType}
                                        >
                                            Change bandtype
                                        </MenuItem>
                                        <MenuItem
                                            id="change-bandDesc-button"
                                            onClick={this._onChangeBandDesc}
                                        >
                                            Change description
                                        </MenuItem>
                                        <MenuItem
                                            id="delete-band-button"
                                            onClick={this._onDeleteBand}
                                        >
                                            Delete band
                                        </MenuItem>
                                    </Menu>
                                ...
                                {!band.bandtype && (
                                    <div>
                                        ...
                                        <FormControl
                                            className={classes.chooseBandType}
                                        >
                                            <Select
                                                className={
                                                    classes.chooseBandTypeSelect
                                                }
                                                native
                                ...
                                <List>
                                    ...
                                    {band.leader.map((member, index) => (
                                        <ListItem key={index} dense>
                                            ...
                                            {member.uid === this.state.user && (
                                                <Tooltip title="Leave band">
                                                    <IconButton
                                                        onClick={() =>
                                                            this._onLeaveLeader(
                                                                member
                                                ...
                                                (member, index) => (
                                                    <ListItem key={index} dense>
                                                        ...
                                                        {member.status ===
                                                            'pending' && (
                                                            <div>
                                                                <Tooltip title="Accept membership request">
                                                                    <IconButton
                                                                        onClick={() =>
                                                                            this._onAccept(
                                                                                member
                                                                ...
                                                                <Tooltip title="Reject membership request">
                                                                    <IconButton
                                                                        onClick={() =>
                                                                            this._onReject(
                                                                                member
                                    ...
                                    <div>
                                        ...
                                        >
                                            ...
                                            >
                                                        ...
                                                        {band.members.map(
                                                            (member, index) => (
                                                                ...
                                                                >
                                                                    ...
                                                                    />
                                                                    {member.admin && (
                                                                        <Tooltip title="Admin. Click to demote">
                                                                            <IconButton
                                                                                onClick={() =>
                                                                                    this._onDemoteAdmin(
                                                                                        member
                                                                    ...
                                                                    {member.supervisor && (
                                                                        <Tooltip title="Conductor. Click to demote">
                                                                            <IconButton
                                                                                onClick={() =>
                                                                                    this._onDemoteSupervisor(
                                                                                        member
                                                                                    )
                                                                                }
                                                                            >
                                                                    ...
                                                                    {member.status ===
                                                                        'member' &&
                                                                        !member.supervisor && (
                                                                            <Tooltip title="Make conductor">
                                                                                <IconButton
                                                                                    onClick={() =>
                                                                                        this._onMakeSupervisor(
                                                                                            member
                                                                                        )
                                                                                    }
                                                                                >
                                                                    ...
                                                                    {member.status !==
                                                                        ...
                                                                        member.uid !==
                                                                            this
                                                                                .state
                                                                                .user && (
                                                                            <Tooltip title="Remove from band">
                                                                                <IconButton
                                                                                    onClick={() =>
                                                                                        this._onRemove(
                                                                                            member
                                                ...
                                                {this.state.checkedMembers &&
                                                    band.onlymembers.length >
                                                        0 && (
                                                        <List>
                                                            {band.onlymembers.map(
                                                                (
                                                                    member,
                                                                    index
                                                                ) => (
                    ...
                    <ChangeBandNameDialog
                        onRef={ref => (this.changeNameDialog = ref)}
                    />
                    <ChangeBandDescDialog
                        onRef={ref => (this.changeDescDialog = ref)}
                    />
                </div>
            );
