...
exports.convertPDF = functions.storage.object().onFinalize((object, context) =>
    __awaiter(this, void 0, void 0, function*() {
        ...
        if (filePath.endsWith('.zip')) {
            ...
            try {
                ...
                yield bucket
                    .file(filePath)
                    .download({ destination: '/tmp/file.zip' });
            ...
            try {
                ...
                yield inputBucket
                    .file(filePath)
                    .download({ destination: '/tmp/score.pdf' });
                // Delete PDF file
                ...
                const pdfInfo = yield new Promise(resolve =>
                    __awaiter(this, void 0, void 0, function*() {
                        const promise = child_process_promise_1.spawn(
                            'xpdf/pdfinfo',
                            ['-cfg', '/tmp/.xpdfrc', '/tmp/score.pdf']
                        );
                ...
                // Create document
                const pdfRef = yield admin
                    .firestore()
                    .collection(`bands/${bandId}/pdfs`)
                    .add({
                        name: fileName,
                ...
                // Generate images
                const gsProcess = yield child_process_promise_1.spawn(
                    'ghostscript/bin/./gs',
                    [
                        '-dBATCH',
                        '-dNOPAUSE',
                ...
                const convertProcess = yield child_process_promise_1.spawn(
                    'mogrify',
                    [
                        '-crop',
                        '4000x666+0+0',
                ...
                const upload = outputType =>
                    __awaiter(this, void 0, void 0, function*() {
                        // Read files
                        const fileNames = yield fs.readdir(
                            `/tmp/output-${outputType}`
                        );
                        const uploadResponses = yield Promise.all(
                            fileNames.map((name, index) =>
                                pdfBucket.upload(
                                    `/tmp/output-${outputType}/${name}`,
                                    {
                ...
                const croppedPageUrls = (yield upload('cropped')).map(
                    ([url]) => url
                );
                const originalPageUrls = (yield upload('original')).map(
                    ([url]) => url
                );
                // Add page documents
                const pages = [];
                ...
                const process2 = yield child_process_promise_1.spawn(
                    'xpdf/pdftotext',
                    ['-cfg', '/tmp/.xpdfrc', '/tmp/score.pdf']
                );
                ...
                if (true) {
                    ...
                    const composerResult = yield composerPattern.exec(
                        _pages[0]
                    );
                    ...
                    for (let i = 0; i < _pages.length - 1; i++) {
                        ...
                        for (let pattern of patterns) {
                            ...
                            if (patternMatch && !pattern.expr.exec(page)[1]) {
                                detectedInstrNames.push(
                                    pattern.expr.exec(page)[0]
                                );
                            }
                        }
...
exports.createThumbnail = functions.firestore
    .document('bands/{bandId}/scores/{scoreId}')
